...
################################################################################
# CODE FOR RESULTS IN TABLE 3 (PAGE 17) CHICK-REARING
...
data<- read.csv("Table3/Coquet_Roseate_CR.csv") 
...
data<-read.csv('Table3/Coquet_Common_CR.csv')
# data preparation for HMM analysis and validation
dnew<- data %>%
  subset(select= +c(ID,SPECIES,TSECS,LATITUDE,LONGITUDE, DIST2COL,
                    CONTBEH, Observed_2State)) %>%
...
data<-read.csv('Table3/Coquet_Sandwich_CR.csv')
# data preparation for HMM analysis and validation
dnew<- data %>%
  subset(select= +c(ID,SPECIES,TSECS,LATITUDE,LONGITUDE, DIST2COL,
                    CONTBEH, Observed_2State)) %>%
...
data<-read.csv("Table3/Coquet_Arctic_CR.csv")
# data preparation for HMM analysis and validation
dnew<- data %>%
  subset(select= +c(ID,SPECIES,TSECS,LATITUDE,LONGITUDE, DIST2COL,
                    CONTBEH, Observed_2State)) %>%
...
data<-read.csv("Table3/IsleMay_Arctic_CR.csv")
# data preparation for HMM analysis and validation
dnew<- data %>%
...
data<-read.csv('Table3/Cemlyn_Arctic_CR.csv')
# data preparation for HMM analysis and validation
dnew<- data %>%
...
data<-read.csv('Table3/Cockle_Sandwich_CR.csv')
# data preparation for HMM analysis and validation
dnew<- data %>%
...
data<-read.csv('Table3/BlueCircle_Sandwich_CR.csv')
# data preparation for HMM analysis and validation
dnew<- data %>%
  subset(select= +c(ID,SPECIES,TSECS,LATITUDE,LONGITUDE, DIST_COL_K,
                    CONTBEH, observed_state)) %>%
...
data<-read.csv('Table3/SouthShian_Common_CR.csv')
dnew<- data %>%
  subset(select= +c(ID,SPECIES,TSECS,LATITUDE,LONGITUDE, DIST2COL,
                    CONTBEH, observed_state)) %>%
  rename(observedS = observed_state) %>%
...
data<-read.csv('Table3/Cemlyn_Common_CR.csv')
dnew<- data %>%
  subset(select= +c(ID,SPECIES,TSECS_GMT,LATITUDE,LONGITUDE, DIST2COL,
                    Continuous, Observed_2State)) %>%
  rename(observedS = Observed_2State) %>%
...
data<-read.csv('Table3/GlasEileanan_Common_CR.csv')
dnew<- data %>%
  subset(select= +c(ID,SPECIES,TSECS,LATITUDE,LONGITUDE, DIST2COL,
                    CONTBEH, observed_state)) %>%
  rename(observedS = observed_state) %>%
...
data<-read.csv('Table3/Leith_Common_CR.csv')
# data preparation for HMM analysis and validation
dnew<- data %>%
...
data<- read.csv("Table3/Forvie_Sandwich_CR.csv")
# data preparation for HMM analysis and validation
dnew<- data %>%
