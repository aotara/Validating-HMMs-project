################################################################################
#     CODE FOR FIGURES 4 AND 5 ON PAGE 17
################################################################################
# load libraries
library(NISTunits)
library(momentuHMM)
...
######################  STEP 4:  FIGURE 5 Page 17 ##############################
# ID 87
# filter data based on ID
id87<- db%>% filter(ID==87)
write.csv(id87, file ='id87-boat-approximate-tern-location.csv',row.names=F) 
id87<-read.csv('id87-boat-approximate-tern-location.csv')
# subset with boat latitude and longitude columns
d1a<- subset(id87, select = -c(Lon_T,Lat_T) )
# subset with approximate tern latitude and longitude columns 
d1b<- subset(id87, select = -c(Longitude,Latitude) )
# rename column of tern position data
names(d1b)[names(d1b) == 'Lat_T'] <- 'Latitude'
names(d1b)[names(d1b) == 'Lon_T'] <- 'Longitude'
# calculate step length and turning angle for boat and tern tracks
...
# plot figure 4
# plot boat and approximate tern track
plot(id87$Longitude,id87$Latitude, type="o", pch="o",lty=1,col='darkblue',
     main='ID 87', cex=1.5, lwd=1.8,xlab='Longitude', ylab='Latitude',
     cex.lab=1.8, cex.axis=1.7)                                    #boat
points(id87$Lon_T,id87$Lat_T, col="steelblue1",pch="+",cex=1.7)    #tern
lines(id87$Lon_T,id87$Lat_T, col='steelblue1',lty=2, lwd=2)
legend("bottomleft",legend=c("boat track","approximate tern track"), 
       col=c("darkblue","steelblue1"),
       pch=c("o","+"),lty=c(1,2), ncol=1,lwd=1.5,cex=1.5)
# plot histogram of step length for boat and approximate tern track
hist(p1a$step, col = "grey60", border = "black", las = 1, main='',
