...
################################################################################
# CODE FOR RESULTS IN TABLE 2 (PAGE 16) INCUBATION
...
data<-read.csv('Table2/Cockle_Arctic_IN.csv')
View(data)
...
data<-read.csv('Table2/IsleMay_Arctic_IN.csv')
# data preparation for HMM analysis and validation
dnew<- data %>%
...
data<-read.csv('Table2/BlueCircle_Roseate_IN.csv')
# data preparation for HMM analysis and validation
dnew<- data %>%
  subset(select= +c(ID,SPECIES,TSECS,LATITUDE,LONGITUDE, DIST_COL_K,
                    CONTBEH, observed_state)) %>%
...
data<-read.csv('Table2/Leith_Common_IN.csv')
# data preparation for HMM analysis and validation
dnew<- data %>%
...
data<-read.csv('Table2/Cockle_Sandwich_IN.csv')
# data preparation for HMM analysis and validation
dnew<- data %>%
  subset(select= +c(ID,SPECIES,TSECS,LATITUDE,LONGITUDE, DIST_COL_K,
                    CONTBEH, observed_state)) %>%
